...
export class WebService {
  ...
  getDrivers(
    ...
    return this.http
      ...
      .pipe(
        map((res) =>
          res.MRData.StandingsTable.StandingsLists[0].DriverStandings.sort(
            (a, b) => a.position - b.position
          ).map((e) => {
            const driver: Driver = e.Driver;
  ...
  getTeams(
    ...
    return this.http
      ...
      .pipe(
        map((res) =>
          res.MRData.StandingsTable.StandingsLists[0].ConstructorStandings.sort(
            (a, b) => a.position - b.position
          ).map((e) => {
    ...
    return this.http
      ...
      .pipe(
        map((res) => {
          ...
          for (const item of res.MRData.StandingsTable.StandingsLists[0]
            .DriverStandings) {
            stArr.push({
  ...
  getRaceResult(
    ...
    return this.http
      .get<any>(
        proxy +
          `http://ergast.com/api/f1/${season}/${round}/results.json?limit=${limit}&offset=${offset}`,
        httpOptions
      )
      .pipe(
        map((res) => {
          let raceResult: RaceResult;
          const e = res.MRData.RaceTable.Races[0];
          if (e) {
            raceResult = {
              ...
              drivers: e.Results.map((driverResult) => {
                const newDr: DriverResult = driverResult.Driver;
  ...
  getSeasonResults(season: string, limit: number = 1000, offset: number = 0) {
    return this.http.get<any>(
      proxy +
        `http://ergast.com/api/f1/${season}/results.json?limit=${limit}&offset=${offset}`,
      httpOptions
    );
  }
  ...
  getDriverChampionshipPositions(
    driverId: string,
    limit: number = 1000,
    offset: number = 0
  ): Observable<DriverPosition[]> {
    return this.http
      .get<any>(
      ...
      .pipe(
        map((res) => {
          ...
          for (const standing of res.MRData.StandingsTable.StandingsLists) {
            result.push({
