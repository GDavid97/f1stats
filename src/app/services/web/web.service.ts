...
export class WebService {
  ...
  getDriverStanding(season: string = 'current', round: string = 'last', limit: number = 1000, offset: number = 0): Observable<Standing[]> {
    return this.http.get<any>(
      ...
      for (const item of res.MRData.StandingsTable.StandingsLists[0].DriverStandings) {
        stArr.push({
  ...
  getTeamStanding(season: string = 'current', round: string = 'last', limit: number = 1000, offset: number = 0): Observable<Standing[]> {
    return this.http.get<any>(
      ...
      const stArr: Standing[] = [];
      const year = season == 'current' ? res.MRData.StandingsTable.season : season;
      if (!res.MRData.StandingsTable.StandingsLists[0]) {
        return [];
      }
      for (const item of res.MRData.StandingsTable.StandingsLists[0].ConstructorStandings) {
        stArr.push({
